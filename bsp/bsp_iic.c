...
void IICTransmit(IICInstance* iic, uint8_t *pData, uint16_t Size, IIC_Work_Mode IIC_Seq)
{
  ...
  switch (iic->Work_Mode)
    ...
    {
      HAL_I2C_Master_Transmit(iic->bsp_iic_Handle ,  iic->reg , pData, Size, 100 );
    } 
  ...
  switch (iic->Work_Mode)
	{
	 case IIC_BLACK_MODE://阻塞模式
  	 if(IIC_Seq== IIC_SEQ_RELEASE)
    {
      HAL_I2C_Master_Receive(iic->bsp_iic_Handle ,  iic->reg , pData, Size, 100 );
    } 
     else if(IIC_Seq== IIC_SEQ_HOLDON)
	  {
	  	HAL_I2C_Master_Receive(iic->bsp_iic_Handle ,  iic->reg , pData, Size, 100 );
...
{
	case IIC_Write_Mem:
		HAL_I2C_Mem_Write(iic->bsp_iic_Handle ,iic->reg, mem_addr,bit_flag, pData ,Size, 100);
    break;
   
	case IIC_Read_Mem:
		HAL_I2C_Mem_Read(iic->bsp_iic_Handle ,iic->reg, mem_addr, bit_flag, pData ,Size, 100);
    break;
